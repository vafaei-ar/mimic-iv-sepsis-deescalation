#!/usr/bin/env python3
"""Bootstrap inference for frozen PSU modified external replication.

Reuses the frozen PSU PS source and point-estimate outcome construction. Performs
1000 encounter-level nonparametric bootstrap replicates, refitting the frozen PS
within each replicate, then recomputes primary stabilized ATE IPTW plus overlap and
weight-truncation sensitivities. Aggregate outputs only.
"""
...
def _worker(task):
    ...
    try:
        ...
        q01,q99 = np.quantile(sw,[.01,.99]); w199=np.clip(sw,q01,q99)
        q025,q975 = np.quantile(sw,[.025,.975]); w25975=np.clip(sw,q025,q975)
        methods = {
        ...
        for mname,w in methods.items():
            for outcome,kind in _BOOT_OUTCOMES:
                ...
                mu1=_wmean(z[y==1],w[y==1]); mu0=_wmean(z[y==0],w[y==0])
                rows.append({"rep":rep,"method":mname,"outcome":outcome,"type":kind,
                             "deescalated_mean_or_risk":mu1,"continued_mean_or_risk":mu0,
...
def run_bootstrap(df, binary, continuous, effect_outcomes, outdir):
    ...
    with ProcessPoolExecutor(max_workers=jobs,mp_context=ctx,initializer=_init_worker,
                             initargs=(df,binary,continuous,effect_outcomes)) as ex:
    ...
    for (method,outcome),sub in boot.groupby(["method","outcome"],sort=False):
        ...
        if kind=="binary": metrics.append("risk_ratio_A1_over_A0")
        for metric in metrics:
            vals=pd.to_numeric(sub[metric],errors="coerce").replace([np.inf,-np.inf],np.nan).dropna()
            ci.append({"method":method,"outcome":outcome,"estimand":metric,
                       "lower_95":float(vals.quantile(.025)),"upper_95":float(vals.quantile(.975)),
                       "n_success":int(len(vals)),"n_requested":REPS})
    ...
    diag={"n_requested":REPS,"n_successful_replicates":REPS-len(failures),"n_failed_replicates":len(failures),
          "jobs":jobs,"seed":SEED,"glm_maxiter":GLM_MAXITER,
