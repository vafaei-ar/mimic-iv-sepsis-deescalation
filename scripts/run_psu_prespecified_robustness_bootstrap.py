...
Reuses the already frozen robustness variants:
1) strict cohort with MED_ADMIN day-3 exposure reclassification;
2) lenient 96-hour landmark with PRESCRIBING exposure.

For each variant, the frozen covariate/PS/outcome definitions are preserved, the PS is
refit in each of 1000 encounter-level nonparametric bootstrap replicates, and aggregate
confidence intervals are exported. No row-level artifacts are written.
"""
