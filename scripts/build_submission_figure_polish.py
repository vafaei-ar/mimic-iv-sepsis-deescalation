...
def build_fig1() -> None:
    ...
    savefig(fig, "Fig1_target_trial_timeline_submission")
...
def build_fig2() -> None:
    ...
    for x, l, h, yy in zip(est, lo, hi, y):
        ax.text(x, yy + 0.16, f"{x:+.2f} ({l:+.2f}, {h:+.2f})", ha="center", va="bottom", fontsize=8.4)
    savefig(fig, "Fig2_progressive_adjustment_submission")
...
def build_fig3() -> None:
    ...
    for j, (key, title, unit) in enumerate(panels):
        ...
        for x, l, h, y0 in zip(vals, los, his, yy):
            if decimals == 2:
                txt = f"{x:+.2f} ({l:+.2f}, {h:+.2f})"
            else:
                txt = f"{x:+.3f} ({l:+.3f}, {h:+.3f})"
            ax.text(x, y0 + 0.16, txt, ha="center", va="bottom", fontsize=8.1)
    fig.suptitle("Cross-dataset mortality and stewardship outcomes", fontsize=11.5, y=1.01)
    savefig(fig, "Fig3_cross_dataset_outcomes_submission")
...
PRETTY_LABELS = {
    ...
    "vent_pre72": "Mechanical ventilation, pre-72 h",
    ...
    "hr_min_pre72": "Minimum heart rate, pre-72 h",
    "platelet_last_worst_48_72h": "Platelet count, worst 48-72 h",
    ...
    "lactate_late_last_48_72h": "Last lactate, 48-72 h",
    "sicu": "Surgical ICU",
    ...
    "severity_index_pre72": "Severity index, pre-72 h",
    "bilirubin_last_worst_48_72h": "Bilirubin, worst 48-72 h",
...
def pretty_label(name: str) -> str:
    ...
    for src, dst in [(" icu", " ICU"), ("icu ", "ICU "), (" wbc", " WBC"), ("wbc ", "WBC "), (" sofa", " SOFA"), ("sofa ", "SOFA "), (" iv", " IV"), ("iv ", "IV ")]:
        text = text.replace(src, dst)
    return text[:1].upper() + text[1:]


def build_esm1() -> None:
    bal = pd.read_csv(OUT / "mimic_primary_balance_before_after.csv")
    ...
    ax.set_yticks(y)
    ax.set_yticklabels([pretty_label(v) for v in top["variable"]], fontsize=8.2)
    ax.set_xlabel("Absolute standardized mean difference")
    ax.set_title("MIMIC-IV primary covariate balance")
    ax.legend(frameon=False, fontsize=9.5)
    savefig(fig, "ESM_Fig1_mimic_balance_love_submission")
...
def build_esm2() -> None:
    ...
    for a, label in [(1, "De-escalated/stopped"), (0, "Continued broad-spectrum")]:
        axes[0].hist(w.loc[w["A"] == a, "ps_den"], bins=np.linspace(0, 1, 31), histtype="step", linewidth=1.6, label=label)
        axes[1].hist(w.loc[w["A"] == a, "SW_A"], bins=30, histtype="step", linewidth=1.6, label=label)
    ...
    savefig(fig, "ESM_Fig2_mimic_ps_weights_submission")
...
def main() -> None:
    ...
    required = [
        ...
        OUT / "mimic_primary_balance_before_after.csv",
        COHORT,
    ]
