...
Runs two sensitivities that were identified before outcome inspection:
1) MED_ADMIN-based day-3 broad-spectrum exposure classification on the frozen strict cohort.
2) Lenient 96-hour landmark eligibility using >= landmark calendar date rather than >.

The frozen covariate/PS specification and frozen PSU outcome definitions are otherwise
reused unchanged. Aggregate outputs only. No bootstrap inference in this task.
"""
...
MEDADMIN_OVERRIDE = r'''
    # Prespecified MED_ADMIN exposure sensitivity. MED_ADMIN time fields were found
    # unusable locally, so administration intervals use the validated date-span rule:
    # start date 00:00:00 through stop date 23:59:59 (or start-date end-of-day if stop missing).
    mcols=cols["med"]
...
def main() -> None:
    ...
    rows = []
    rows.append(run_variant(source, "medadmin_exposure", args.data_root, args.output_dir / "medadmin_exposure"))
    rows.append(run_variant(source, "lenient_landmark", args.data_root, args.output_dir / "lenient_landmark"))
