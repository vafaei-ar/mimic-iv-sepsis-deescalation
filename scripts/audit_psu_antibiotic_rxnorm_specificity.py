...
def main() -> None:
    ...
    # Export code-derived context only. This is source-code metadata, never patient data.
    lines = old_text.splitlines()
    context_rows = []
    for code in rxnorm_literals:
        ...
        for i in hits[:3]:
            ...
            context_rows.append({"code": code, "legacy_code_context": context[:500]})
    ...
    overlap = con.execute(f"""
        ...
        u AS (
          SELECT encounterid FROM pe UNION SELECT encounterid FROM me
        )
        SELECT
        ...
        FROM u
        LEFT JOIN pe USING (encounterid)
        LEFT JOIN me USING (encounterid)
    """).fetchdf()
    ...
    summary = {
        ...
        "purpose": "Determine whether legacy PSU RxNorm codes identify a coherent broad-spectrum phenotype despite route fields being predominantly UN, and quantify PRESCRIBING versus MED_ADMIN concordance before freezing source choice.",
        "guardrail": "Diagnostic only; do not freeze source or IV phenotype unless code context and aggregate concordance support it.",
    }
