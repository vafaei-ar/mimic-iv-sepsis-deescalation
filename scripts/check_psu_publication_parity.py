...
# Point estimates and balance diagnostics should agree to at least 1e-5 absolute.
# This is much smaller than any manuscript rounding and still catches meaningful drift.
POINT_ATOL = 1e-5

# Bootstrap percentile endpoints are slightly more sensitive to numerical-library
# differences because tiny replicate-level changes can alter an order statistic near
# the 2.5th/97.5th percentiles. We therefore use outcome-scale tolerances:
#   RD: 0.001 absolute = 0.10 percentage point
#   RR: 0.005 absolute
# Both are far below the precision used for scientific interpretation.
...
def _record(checks: list[dict], name: str, observed, expected, tol=None) -> None:
    """Append one JSON-safe parity check row."""
    if tol is None:
...
def main() -> None:
    ...
    # Run the final public entry points, not private shortcuts. This verifies the same
    # path a collaborator following the README/walkthrough would use.
